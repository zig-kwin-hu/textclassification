...
class LSTM_MODEL(object):
	...
	def __init__(self, is_training, word_embeddings, config):
		...
		lstm_cell = tf.nn.rnn_cell.GRUCell(size,activation = tf.nn.relu)
...
#			embedding = tf.get_variable("embedding", [vocab_size, size])
			embedding = tf.Variable(word_embeddings)
...
#				if(current_step % int(config.vocab_size/config.batch_size) == 0):
#					i += 1
#					tf.assign(lr,0.2 * (0.5 ** i))
			...
			print num
			for i in range(num):
				res = res + dev_step(x_train[i * eval_config.batch_size:(i+1)*eval_config.batch_size], y_train[i * eval_config.batch_size:(i+1)*eval_config.batch_size])
				tot = tot + 1.0
			print res/tot
